...
class GetLineLabelTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parser = TxtParser(verbose=2)
    ...
    def test_is_header(self):
        for s, expected_label in self.strings:
            if expected_label == 'header':
                self.assertTrue(self.parser._is_header(s))
            else:
                self.assertFalse(self.parser._is_header(s))


    def test_is_subheader(self):
        for s, expected_label in self.strings:
            if expected_label == 'subheader':
                self.assertTrue(self.parser._is_subheader(s))
            else:
                self.assertFalse(self.parser._is_subheader(s))


    def test_is_subsubheader(self):
        for s, expected_label in self.strings:
            if expected_label == 'subsubheader':
                self.assertTrue(self.parser._is_subsubheader(s))
            else:
                self.assertFalse(self.parser._is_subsubheader(s))


    def test_is_special_header(self):
        for s, expected_label in self.strings:
            if expected_label == 'special_header':
                self.assertTrue(self.parser._is_special_header(s))
            else:
                self.assertFalse(self.parser._is_special_header(s))


    def test_is_speech_new(self):
        for s, expected_label in self.strings:
            if expected_label == 'speech_new':
                self.assertTrue(self.parser._is_speech_new(s))
            else:
                self.assertFalse(self.parser._is_speech_new(s))
    
    def test_is_speech_ctd(self):
        for s, expected_label in self.strings:
            if expected_label == 'speech_ctd':
                self.assertTrue(self.parser._is_speech_ctd(s))
            else:
                self.assertFalse(self.parser._is_speech_ctd(s))


    def test_is_scene(self):
        for s, expected_label in self.strings:
            if expected_label == 'scene':
                self.assertTrue(self.parser._is_scene(s))
            else:
                self.assertFalse(self.parser._is_scene(s))
